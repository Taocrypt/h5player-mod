// ==UserScript==
// @name         音视频增强脚本（Taocrypt魔改版）：无极调速|长按倍速|快乐刷剧|视频下载|画面截图等「适用大部分网站」
// @name:en      Audio & Video Enhancement (Taocrypt mod): Infinite speed, long-press speed, downloads, screenshots
// @name:zh      音视频增强脚本（Taocrypt魔改版）：支持无极调速、手机长按倍速、快乐刷剧、视频下载、画面截图等「适用大部分网站」
// @name:zh-TW   音影片增強腳本（Taocrypt魔改版）：支援無極調速、手機長按倍速、快樂刷劇、影片下載、畫面截圖等「適用大部分網站」
// @name:ja      オーディオ/ビデオ拡張（Taocrypt改造版）: 無限速度調整、モバイル長押し倍速、快適な視聴、ビデオダウンロード、スクリーンショットなど「多くのサイトに適用可能」
// @name:ko      오디오/비디오 향상 (Taocrypt 모드): 무한 속도 조정, 모바일 길게 누르기 배속, 즐거운 시청, 비디오 다운로드, 스크린샷 지원 (대부분의 웹사이트 적용)
// @name:ru      Улучшение аудио/видео (мод Taocrypt): бесконечная регулировка скорости, ускорение по длительному нажатию, скачивание и др.
// @name:de      Audio-/Video-Enhancement (Taocrypt Mod): Stufenlose Geschwindigkeit, Langdruck-Geschwindigkeit (Mobil), Downloads, Screenshots
// @namespace    https://github.com/xxxily/h5player
// @homepage     https://github.com/xxxily/h5player
// @version      4.3.4
// @description  （Taocrypt魔改版）支持所有H5视频网站；无极调速与手机长按倍速（桥接 h5player 源逻辑）；兼容 DPlayer 自定义控件覆盖；视频截图、画中画、网页全屏、亮度/饱和度/对比度调节、自定义配置等；保留 Shadow DOM/iframe 深度扫描与轻提示/振动反馈；适合倍速学习与刷剧、视频下载等能力
// @description:en  (Taocrypt mod) Supports all H5 video sites; infinite speed + mobile long-press speed (bridged to h5player core); DPlayer overlay compatibility; screenshots, PiP, web fullscreen, brightness/saturation/contrast, custom config; preserves Shadow DOM/iframe deep scan and UI tip/vibrate; great for fast learning and binge watching; includes download capability
// @description:zh  （Taocrypt魔改版）支持所有H5视频网站；无极调速与手机长按倍速（桥接 h5player 源逻辑）；兼容 DPlayer 自定义控件覆盖；视频截图、画中画、网页全屏、亮度/饱和度/对比度调节、自定义配置等；保留 Shadow DOM/iframe 深度扫描与轻提示/振动反馈；适合倍速学习与刷剧，并提供视频下载能力
// @description:zh-TW  （Taocrypt魔改版）支援所有 H5 視頻網站；無極調速與手機長按倍速（橋接 h5player 核心邏輯）；相容 DPlayer 自訂控件覆蓋；影片截圖、畫中畫、網頁全螢幕、亮度/飽和度/對比度調整、自訂配置等；保留 Shadow DOM/iframe 深度掃描與輕提示/震動；適合倍速學習與追劇，並提供影片下載能力
// @description:ja  （Taocrypt改造版）すべての H5 動画サイトに対応。無限速度調整＋モバイル長押し倍速（h5player コアへブリッジ）、DPlayer オーバーレイ互換、スクリーンショット、PiP、Web 全画面、明るさ/彩度/コントラスト調整、カスタム設定等。Shadow DOM/iframe の深い走査や UI 提示/バイブに対応。倍速学習や視聴に最適、ダウンロード機能もあり
// @description:ko  (Taocrypt 모드) 모든 H5 비디오 사이트 지원. 무한 속도 조정 + 모바일 길게 누르기 배속(h5player 코어 브리지), DPlayer 오버레이 호환, 스크린샷, PIP, 웹 전체 화면, 밝기/채도/대비 조정, 사용자 정의 설정 등. Shadow DOM/iframe 심층 스캔 및 UI 팁/진동 유지. 배속 학습과 시청에 적합하며, 다운로드 기능 제공
// @description:ru  (Мод Taocrypt) Поддержка всех H5-видеосайтов; бесконечная регулировка скорости и ускорение по длительному нажатию (связь с ядром h5player); совместимость с оверлеями DPlayer; скриншоты, PiP, полноэкранный веб-режим, яркость/насыщенность/контраст, пользовательские настройки; глубокий обход Shadow DOM/iframe, подсказки и вибрация; подходит для обучения на ускоренной скорости и просмотра, есть загрузка видео
// @description:de  (Taocrypt Mod) Unterstützt alle H5-Videoseiten; stufenlose Geschwindigkeit + Langdruck-Geschwindigkeit (mit h5player-Kern verbunden); DPlayer-Overlay-Kompatibilität; Screenshots, PiP, Web-Vollbild, Helligkeit/Sättigung/Kontrast, benutzerdef. Konfig.; behält Shadow DOM/iframe Tiefenscan und UI-Hinweis/Vibration; ideal für Schnelllernen und Serien, inkl. Download
// @author       Taocrypt (mod) | 原作者: ankvps
...
// @license      GPL
// @modified-by  Taocrypt
// @modified-at  2026-01-03
// @mod-notes    集成长按倍速（手机端）与 h5player 源倍速逻辑协同；增加全局捕获兼容层，支持 DPlayer 等自定义播放器覆盖控件场景；保留 Shadow DOM/iframe 深度扫描与轻提示/振动反馈；不删减原功能代码
// ==/UserScript==
...
/* === 集成：手机长按倍速播放增强（来自2.txt，保留全部功能）=== */
/*
  版本特性（Taocrypt 魔改版）：
  - 共存兼容：长按倍速与 h5player 原快捷键/菜单倍速同时可用，互不干扰
  - 源逻辑桥接：长按倍速优先调用 h5player 的 setPlaybackRate/lockPlaybackRate 流程，确保实际生效
  - 全局捕获层：在 document 捕获阶段监听触摸事件，基于点击坐标定位 video，兼容 DPlayer 等自定义控件覆盖
  - 深度扫描：递归穿透 Shadow DOM 与同源 iframe，尽可能绑定所有视频实例
  - 轻提示与振动：右上角半透明倍速提示，支持 navigator.vibrate（设备兼容视情况）
  - 保留原功能：不删减 h5player 原有功能与配置，完整保留 2.txt 手势逻辑
*/
(function () {
    'use strict';
    ...
    function showSpeedTip(speed) {
        ...
        if (!tip || !root.contains(tip)) {
            ...
            tip.style.cssText = `
                ...
                padding: 6px 12px !important; border-radius: 16px !important;
                font-size: 16px !important; font-weight: bold !important;
                z-index: 2147483647 !important; pointer-events: none !important;
                transition: opacity 0.4s !important; backdrop-filter: blur(4px) !important;
            `;
    ...
    // 兼容层：统一设置播放速率（避免被宿主脚本锁定或拦截）
    function setPlaybackRateCompat(video, rate) {
        if (!video) return;
        // 优先调用 h5player 的统一倍速设置API，避免与其劫持/锁机制冲突
        try {
            // 优先通过 h5player 实例协同
            const t = (window._h5Player && typeof window._h5Player.setPlaybackRate === 'function') ? window._h5Player : (typeof h5Player !== 'undefined' ? h5Player : null);
            if (t && typeof t.setPlaybackRate === 'function') {
                try {
                    // 将当前触发的 video 作为活动播放器实例，避免多视频页面错绑
                    if (video && t.playerInstance !== video) {
                        t.playerInstance = video;
                        // 初始化该实例的代理与锁逻辑（不破坏原脚本）
        ...
        // 通知可能存在的宿主脚本（如 h5player）协调设置倍速（若无API，则走事件协商）
        try {
    ...
    function attachToVideo(video) {
        if (video._longPressAttached) return;
        video._longPressAttached = true;
        ...
        video.addEventListener('touchstart', e => {
            ...
            pressTimer = setTimeout(() => {
                ...
                setPlaybackRateCompat(video, currentSpeed);
                showSpeedTip(currentSpeed);
        ...
        video.addEventListener('touchmove', e => {
            ...
            if (steps !== 0) {
                ...
                setPlaybackRateCompat(currentVideo, currentSpeed);
                showSpeedTip(currentSpeed);
                startY = touch.clientY;
            }
        ...
        video.addEventListener('touchend', () => {
            ...
            if (isLongPressing && currentVideo) {
                setPlaybackRateCompat(currentVideo, originalSpeed);
    ...
    // 递归绑定（支持 Shadow DOM）
    function attachToVideos(root) {
        if (!root) return;
        root.querySelectorAll('video').forEach(attachToVideo);

        root.querySelectorAll('*').forEach(el => {
            if (el.shadowRoot) {
                attachToVideos(el.shadowRoot);
            }
        });
    }
        ...
        document.querySelectorAll('iframe').forEach(iframe => {
            try {
                if (iframe.contentDocument) {
                    attachToVideos(iframe.contentDocument);
                }
            } catch (e) { /* 跨域忽略 */ }
        });
    }
    ...
    // 持续监听
    new MutationObserver(deepScan).observe(document.documentElement || document.body, {
        childList: true,
        subtree: true
    });
    ...
    ['fullscreenchange', 'webkitfullscreenchange'].forEach(evt => {
        document.addEventListener(evt, () => setTimeout(() => showSpeedTip(currentSpeed || 1.0), 300));
    });

    // 兼容层：针对自定义播放器（如 DPlayer）覆盖 video 导致触摸事件无法直达 video 的情况
    // 思路：在捕获阶段于 document 级别监听触摸事件，基于坐标查找对应的 video 并执行同样的长按倍速逻辑
    (function setupGlobalLongPressCompat(){
        let globalPressTimer = null;
        let globalActive = true; // 可按需置为 false 来关闭全局兼容层
        function findVideoByPoint(x, y){
            const candidates = [];
                ...
                try{
                    root.querySelectorAll('video').forEach(v=>{
                        ...
                        if (rect && rect.width > 30 && rect.height > 30){
                            if (x >= rect.left && x <= rect.right && y >= rect.top && y <= rect.bottom){
                                candidates.push({el:v, area: rect.width * rect.height});
                            }
                        }
                    });
                    // 递归 Shadow DOM
        ...
        document.addEventListener('touchstart', function(e){
            ...
            if (e.touches.length !== 1 || isLongPressing) return;
            const touch = e.touches[0];
            const video = findVideoByPoint(touch.clientX, touch.clientY);
            if (!video) return;
            const rect = video.getBoundingClientRect();
            const x = touch.clientX - rect.left;
            const width = rect.width;
            ...
            currentVideo = video;
            originalSpeed = video.playbackRate || 1.0;
            if (globalPressTimer) clearTimeout(globalPressTimer);
